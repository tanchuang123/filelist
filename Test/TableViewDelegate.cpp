...
//        this->loadStyleSheet(":/qss/myStyleSheet.qss");
    ...
    m_parent = dynamic_cast<CustomTableView*>(parent);//强制类型转换为CustomListView
...
//绘制按钮
void TableViewDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const//QStyledItemDelegate是专门用于项的(item)的视图委托
{
    painter->save();//保存绘制状态
...
void TableViewDelegate::setType(TableViewDelegate::Type type)
{
    m_type = type;
}

void TableViewDelegate::setButtonList(const QList<QPushButton *> &value)
{
    ButtonList = value;
}
